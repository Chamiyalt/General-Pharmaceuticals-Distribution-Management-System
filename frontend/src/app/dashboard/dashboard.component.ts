...
import { GetExpiresoonDrugsService } from 'app/get-expiresoon-drugs.service';
import { GetHosStockService } from "../get-hos-stock.service";
import { hosStock } from "app/getHosStock.model.1";
...
export class DashboardComponent implements OnInit {
  ...
  private GetExpiresoonDrugsService: Subscription;
  ...
  constructor(
    ...
    public DrugService: GetExpiresoonDrugsService,
    public GetHosStockServic: GetHosStockService
  ) { }
  ...
  ngOnInit() {
    ...
    this.GetExpiresoonDrugsService = this.DrugService.getDrugUpdateListener()
    ...
    this.GetHosStockService = this.GetHosStockServic.getStockUpdateListener()
      .subscribe((hos: hosStock[]) => {
        ...
        for (var x = 0; x < hos.length; x++) {
          ...
          if (Number(Number(hos[x].quentity) / Number(hos[x].preQty) * 100) < 30) {
            let modelData2 = {
              id: hos[x].id,
              Hname: hos[x].Hname,
  ...
  showNotification(from, align) {
    ...
    $.notify({
      icon: "danger",
      message: "Check the expiration date on your medicines."
  ...
  showNotification2(from, align) {
    ...
    $.notify({
      icon: "danger",
      message: "Check the Stocks on your medicines."
